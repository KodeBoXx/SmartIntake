...
public final class FormRuntime {
  ...
  private void validateValue(JsonNode f,JsonNode value){String type=f.path("type").asText();if("multiChoice".equals(type)){if(!value.isArray())throw new IllegalArgumentException("INVALID_LITERAL");for(JsonNode x:value)choice(f,x);return;}if("choice".equals(type)){choice(f,value);return;}if("object".equals(type)){if(!value.isObject())throw new IllegalArgumentException("INVALID_LITERAL");return;}if("list".equals(type)||"repeater".equals(type)){if(!value.isArray())throw new IllegalArgumentException("INVALID_LITERAL");return;}if("text".equals(type)){if(!value.isTextual())throw new IllegalArgumentException("INVALID_LITERAL");int min=f.path("constraints").path("minLength").asInt(0),max=f.path("constraints").path("maxLength").asInt(Integer.MAX_VALUE);if(value.asText().length()<min||value.asText().length()>max)throw new IllegalArgumentException("TEXT_LENGTH");return;}ContractValue.validate(type,value);}
