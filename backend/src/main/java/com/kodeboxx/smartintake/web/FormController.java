...
public class FormController {
 ...
 @PostMapping("/{form}/releases") public ResponseEntity<?> publish(@PathVariable String workspace,@PathVariable UUID form,@RequestParam(required=false) UUID reviewRequestId,@RequestHeader(value="X-Staff-Session",required=false) String token){return intake.canonicalForm(form) ? governed.publish(workspace,form,reviewRequestId,token) : intake.publish(workspace,form,token);}
}
