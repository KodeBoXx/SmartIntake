...
public class RespondentSessionController {
 ...
 @PostMapping("/public/channels/{channel}/bootstrap") public ResponseEntity<?> bootstrap(@PathVariable UUID channel,@RequestBody Map<String,Object> in){String parent=String.valueOf(in.get("parentOrigin"));return ResponseEntity.ok().header("Content-Security-Policy","default-src 'self'; base-uri 'self'; frame-ancestors "+parent+"; object-src 'none'").body(intake.bootstrapChannel(channel,parent));}
