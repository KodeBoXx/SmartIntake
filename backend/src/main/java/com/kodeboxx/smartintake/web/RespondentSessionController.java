...
public class RespondentSessionController {
 ...
 @GetMapping(value="/public/channels/{channel}/embed",produces="text/html") public ResponseEntity<String> embed(@PathVariable UUID channel,@RequestParam String parentOrigin){return ResponseEntity.ok().header("Content-Security-Policy","default-src 'self'; frame-ancestors "+parentOrigin+"; object-src 'none'").body(intake.embedChannel(channel,parentOrigin));}
