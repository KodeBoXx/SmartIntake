...
function applyCommand(document: AuthoringDocument, command: AuthoringCommand): AuthoringDocument {
  ...
  if (command.type === 'remove-page' && command.targetId) return { ...document, phases: document.phases.map((phase) => ({ ...phase, pages: phase.pages.filter((page) => page.id !== command.targetId) })) };
