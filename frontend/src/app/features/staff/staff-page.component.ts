...
import { CuiAlertComponent, CuiButtonComponent, CuiCardComponent, CuiDataTableComponent, CuiDrawerComponent, CuiEmptyStateComponent, CuiPopoverComponent, CuiStatsStripComponent, CuiDropdownComponent, CuiConfirmDialogComponent, CuiIconComponent, CuiNavItemComponent, CuiToastService } from '@certinal/ui';
...
@Component({
  standalone: true,
  imports: [CuiAlertComponent, CuiButtonComponent, CuiCardComponent, CuiDataTableComponent, CuiDrawerComponent, CuiEmptyStateComponent, CuiStatsStripComponent, CuiPopoverComponent, CuiDropdownComponent, CuiConfirmDialogComponent, CuiIconComponent, CuiNavItemComponent],
  template: `
    <section data-testid="staff-page" [attr.data-state]="state">
      <nav data-testid="staff-mid-nav" aria-label="Primary navigation" class="hidden min-[900px]:flex lg:hidden items-center gap-1 border-b border-border-default bg-bg-surface px-2 py-2 mb-4"><cui-nav-item label="Forms" icon="file-text" (navClick)="navigate('/workspaces/demo/forms')" /><cui-nav-item label="Responses" icon="inbox" (navClick)="navigate('/workspaces/demo/submissions')" /><cui-nav-item label="Settings" icon="settings" (navClick)="navigate('/settings/organization')" /></nav>
      <div class="flex flex-wrap items-center justify-between gap-3"><div><p class="type-caption">STAFF WORKSPACE</p><h1 class="type-h3">{{ title }}</h1><p class="type-caption" data-testid="state-evidence">State: {{ state }}</p></div>@if (primaryAction) { <cui-button [disabled]="!actionsAllowed" (buttonClick)="edit()"><span class="flex items-center gap-1.5"><cui-icon name="pencil" size="sm" />{{ primaryAction }}</span></cui-button> }</div>
...
export class StaffPageComponent {
  ...
  get actionsAllowed(): boolean { return this.state !== 'no-side-effects'; }
  ...
  isAlertState(): boolean { return ['invalid', 'denied', 'no-access', 'expired', 'email-unavailable', 'conflict', 'throttled'].includes(this.state); }
