...
export class ResponseStaffPageComponent {
  readonly api = inject(SmartIntakeApiService); readonly session = inject(StaffSessionStore); readonly screen = inject(ActivatedRoute).snapshot.data['screen'] as string;
  ...
  private load(exports = false): void { const workspace = this.session.currentWorkspaceId(); if (!workspace) { this.state.set('empty'); return; } if (exports ? !this.canExport() : !this.canView()) { this.state.set('denied'); return; } const request = exports ? this.api.exportResponses(workspace) : this.api.listResponses(workspace); request.subscribe({ next: (items) => { this.responses.set(items); this.state.set(items.length ? 'ready' : 'empty'); }, error: (error) => this.fail(error.status) }); }
