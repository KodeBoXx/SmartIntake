...
@if (delivery().activationCopyLink) { <cui-alert variant="warning" title="Authorized activation link">Deliver this one-time activation link only to the authorized recipient: {{ delivery().activationCopyLink }}</cui-alert> }
@if (delivery().temporaryPasswordCopy) { <cui-alert variant="warning" title="Authorized temporary password">Deliver this temporary password only to the authorized recipient: {{ delivery().temporaryPasswordCopy }}</cui-alert> }
...
export class AdministrationPageComponent {
  ...
  readonly screen = this.route.snapshot.data['screen'] as string; readonly state = signal<ViewState>('loading'); readonly message = signal(''); readonly organizations = signal<any[]>([]); readonly users = signal<any[]>([]); readonly policies = signal<unknown[]>([]); readonly delivery = signal<AuthorizedDeliveryCopies>({});
  private readonly retryKeys = new RetainedMutationKeys();
  organizationName = ''; ownerEmail = ''; email = ''; role = 'member'; workspaceRole = 'author'; readonly roleOptions = [{ label: 'Member', value: 'member' }, { label: 'Administrator', value: 'administrator' }]; readonly workspaceRoleOptions = WORKSPACE_ROLES.map((role) => ({ label: role, value: role }));
  ...
  saveOrganization(): void {
    ...
    const revision = this.organizations().find((candidate) => candidate.id === organization?.organizationId)?.revision;
    if (!organization || !this.organizationName.trim() || !Number.isInteger(revision)) { this.state.set('invalid'); this.message.set('Reload the organization resource before saving so its current revision can be protected.'); return; }
    const body = { name: this.organizationName }; const action = 'save-organization'; const resource = `platform/organizations/${organization.organizationId}`;
