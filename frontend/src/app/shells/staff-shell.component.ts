...
import { CuiAppShellComponent, CuiHeaderComponent, CuiHeaderDrawerDirective, CuiHeaderTabsDirective, CuiNavItemComponent, CuiSidebarShellComponent } from '@certinal/ui';

@Component({
  standalone: true,
  imports: [RouterOutlet, RouterLink, CuiAppShellComponent, CuiHeaderComponent, CuiHeaderDrawerDirective, CuiHeaderTabsDirective, CuiNavItemComponent, CuiSidebarShellComponent],
  template: `
    <cui-app-shell #staffAppShell data-testid="staff-shell" [desktopBreakpoint]="899">
      ...
      <cui-header #staffHeader header role="presentation">
        ...
        <nav avatar aria-label="Primary navigation" class="hidden min-[900px]:flex items-center gap-1">
        ...
        <ng-template cuiHeaderTabs>
          <nav aria-label="Primary navigation" class="flex h-full items-center gap-1">
            <cui-nav-item label="Forms" icon="file-text" [active]="isFormsActive()" (navClick)="go('/workspaces/demo/forms')" />
            <cui-nav-item label="Responses" icon="inbox" [active]="isResponsesActive()" (navClick)="go('/workspaces/demo/submissions')" />
            <cui-nav-item label="Settings" icon="settings" [active]="isSettingsActive()" (navClick)="go('/settings/organization')" />
          </nav>
      ...
      <cui-sidebar-shell sidebar [collapsed]="staffAppShell.sidebarCollapsed()">
