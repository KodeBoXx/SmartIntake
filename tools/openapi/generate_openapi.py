...
def resource_example(resource: str) -> dict[str, Any]:
    ...
    extras: dict[str, dict[str, Any]] = {
        ...
        "Interpretation": {"submissionId": "submission-01J2W5RFR3K24SFWDX2C0N9VW3", "score": "12.50"}, "AccountAction": {"action": "recovery-requested"}, "StaffSession": {"userId": "user-01J2W5RFR3K24SFWDX2C0N9VW3", "expiresAt": "2026-09-19T00:00:00Z", "csrfToken": "csrf-01J2W5RFR3K24SFWDX2C0N9VW3"},
    }
...
def build_components(members: list[dict[str, Any]]) -> dict[str, Any]:
    schemas: dict[str, Any] = {
        ...
        "CanonicalInt64": {"type": "string", "pattern": "^-?(?:0|[1-9][0-9]*)$", "description": "Canonical base-10 int64 string; JSON number is forbidden."},
        "CanonicalDecimal": {"type": "string", "pattern": "^-?(?:0|[1-9][0-9]*)(?:\\.[0-9]+)?$", "description": "Canonical base-10 decimal string; JSON number is forbidden."},
        ...
        "PublishedSchema": closed(["$schema", "$id", "type"], {"$schema": {"const": "https://json-schema.org/draft/2020-12/schema"}, "$id": {"type": "string", "format": "uri"}, "type": {"type": "string"}, "title": {"type": "string"}}),
    ...
    return {"openapi": "3.1.0", "info": {"title": "Smart Form Builder Lite 4.0.0 OpenAPI components", "version": "4.0.0"}, "components": {"securitySchemes": {"staffCookie": {"type": "apiKey", "in": "cookie", "name": "smartintake_staff", "description": "Opaque HttpOnly staff session cookie."}, "csrfHeader": {"type": "apiKey", "in": "header", "name": "X-CSRF-Token", "description": "Required for cookie-authenticated mutations."}, "respondentSession": {"type": "http", "scheme": "bearer", "bearerFormat": "opaque-session", "description": "Respondent authorization, valid only for its session or receipt."}}, "parameters": {"IfMatch": {"name": "If-Match", "in": "header", "required": True, "schema": {"type": "string", "pattern": '^"[A-Za-z0-9._-]+"$'}, "example": '"rev-7"'}, "IdempotencyKey": {"name": "Idempotency-Key", "in": "header", "required": True, "schema": {"$ref": "#/components/schemas/OpaqueId"}, "example": "idem-01J2W5RFR3K24SFWDX2C0N9VW3"}, "Cursor": {"name": "cursor", "in": "query", "schema": {"type": "string"}, "description": "Opaque snapshot cursor."}, "Limit": {"name": "limit", "in": "query", "schema": {"type": "integer", "default": 50, "minimum": 1, "maximum": 200}}}, "headers": {"ETag": {"schema": {"type": "string"}, "description": "Strong entity tag."}, "Digest": {"schema": {"type": "string"}, "description": "Content digest for immutable publication bytes."}, "ContractSha256": {"schema": {"$ref": "#/components/schemas/Sha256"}, "description": "Published contract SHA-256."}}, "examples": {"Problem": {"value": problem_example}, "CapabilityRegistry": {"value": registry}}, "responses": {}, "schemas": schemas, "x-contract-schema-resources": [{"$ref": f"./{name}"} for name in ["package.schema.json", "expression.schema.json", "input-answer.schema.json", "typed-answer.schema.json", "runtime-manifest.schema.json", "submission-envelope.schema.json", "event.schema.json"]]}}


def add_problem_responses(components: dict[str, Any]) -> None:
    response_codes = {"BadRequest": "Malformed request", "Unauthenticated": "Authentication is required", "Forbidden": "Scope is insufficient", "NotFound": "Resource is absent or concealed", "Conflict": "Replay or state conflict", "PreconditionFailed": "ETag is stale", "PreconditionRequired": "If-Match is required", "Unprocessable": "Semantic validation failed", "RateLimited": "Rate limit exceeded", "Gone": "Session or capability expired", "InternalError": "Unexpected failure"}
    for name, description in response_codes.items():
        components["components"]["responses"][name] = {"description": description, "content": {"application/problem+json": {"schema": {"$ref": "#/components/schemas/Problem"}, "examples": {"problem": {"$ref": "#/components/examples/Problem"}}}}}
...
def operation(member: dict[str, Any], components: dict[str, Any], counted: bool = True) -> dict[str, Any]:
    ...
    if is_mutation(method):
        ...
        result["x-replay"] = "Idempotency-Key replays an equal request's original result; changed-body reuse is 409. Session mutation replay keys are clientMutationId plus body."
